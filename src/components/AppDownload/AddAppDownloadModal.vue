...
const submit = async () => {
  try {
    const res = await request.post(api.APP_DOWNLOAD, dataSubmit);
    if (res.ok) {
      emit("updated");
      notification.success({
        message: $t("app_download.add_success"),
      });
      closeModal();
    } else {
      notification.error({
        message: res.e ?? $t("app_download.add_error"),
      });
    }
  } catch (error) {
    console.log(error);
    notification.error({
      message: error.e ?? $t("app_download.add_error"),
    });
  }
};
</script>
...
<template>
  <a-modal
    ...
    :title="$t('app_download.add_title')"
    :onCancel="closeModal"
    :footer="null"
  >
    <a-form
