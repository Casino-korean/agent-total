...
const submit = async () => {
  try {
    const res = await request.put(api.FANPAGE + "/" + props.data._id, dataSubmit);
    if (res.ok) {
      emit("updated");
      notification.success({
        message: $t("fanpage.edit_success"),
      });
      closeModal();
    } else {
      notification.error({
        message: res.e ?? $t("fanpage.edit_error"),
      });
    }
  } catch (error) {
    console.log(error);
    notification.error({
      message: error.e ?? $t("fanpage.edit_error"),
    });
  }
};
</script>
